...
HTML := $(BUILD_DIR)index.html $(foreach n,1 2 3 4,$(BUILD_DIR)$n_curso/index.html) $(foreach n,1 2 3 4,$(foreach c,1 2,$(BUILD_DIR)$n_curso/$c_cuatri/index.html)) $(foreach asignatura,progi calculo empresas teco fisica,$(BUILD_DIR)1_curso/1_cuatri/$(asignatura)/index.html) $(foreach asignatura,eco redesi progii               sistemas_info algebra,$(BUILD_DIR)1_curso/2_cuatri/$(asignatura)/index.html)
